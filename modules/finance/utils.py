...
from core.config import FINANCE_DATA, FINANCE_DB

VELOCITY_THRESHOLD = 5   # txns per 5-min window
DB_PATH            = str(FINANCE_DB)


# ── Data loading ──────────────────────────────────────────────────────────────
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    txns     = pd.read_csv(FINANCE_DATA / "transactions.csv", parse_dates=["timestamp"])
    accounts = pd.read_csv(FINANCE_DATA / "accounts.csv")
...
def load_invoices() -> pd.DataFrame:
    df = pd.read_csv(FINANCE_DATA / "invoices.csv", parse_dates=["date"])
    return df
