...
from modules.finance.utils import load_invoices, build_invoice_risk_report
...
with tab3:
    ...
    if not st.session_state.invoice_messages:
        ...
        for i, sug in enumerate(SUGGESTIONS):
            with sug_cols[i]:
                if st.button(sug, key=f"inv_sug_{i}", width="stretch"):
    ...
    if getattr(st.session_state, "_inv_run_agent", None):
        ...
        with st.spinner("Running 3-stage analysis (Data → Audit → Synthesis)..."):
            try:
                ...
                err = f"Agent error: {e}. Check your Azure OpenAI credentials in .env."
